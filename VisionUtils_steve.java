...
** Jim Keys, Kar-Wai Lim, Zain Mubashir, Jochen Renz
** All rights reserved.
...
public class VisionUtils {
    ...
    public static int[][] computeDistanceMap(int[][] image) {
        ...
        for (int y = 0; y < image.length; y++) {
            for (int x = 0; x < image[y].length; x++) {
                if (image[y][x] != 0) {
                    ...
                    if (y > 0) image[y][x] = Math.min(image[y][x], image[y-1][x] + 1);
                    if (x > 0) image[y][x] = Math.min(image[y][x], image[y][x-1] + 1);
                }
            }
        }
        ...
        for (int y = image.length - 1; y >= 0; y--) {
            for (int x = image[y].length - 1; x >= 0; x--) {
                if (y < image.length - 1) image[y][x] = Math.min(image[y][x], image[y+1][x] + 1);
                if (x < image[y].length - 1) image[y][x] = Math.min(image[y][x], image[y][x+1] + 1);
            }
        }
    ...
    public static BufferedImage drawBoundingBoxes(BufferedImage canvas, Rectangle[] boxes, Color fgColour, Color bgColour) {
        Graphics2D g2d = canvas.createGraphics();
    ...
    // draws bounding boxes onto an image
    public static BufferedImage drawBoundingBoxes(BufferedImage canvas, List<Rectangle> boxes, Color fgColour, Color bgColour) {
        Graphics2D g2d = canvas.createGraphics();
