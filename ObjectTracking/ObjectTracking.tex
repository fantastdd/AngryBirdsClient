...
We call the movement approximating problem we want to solve MQSR($\Theta$, QCN, $M$) where $\Theta$ is a set of objects in a spatial scenario and QCN is the corresponding EGSR constraint network over the objects. $M$ is a reasoning model composed of a set of configurations written in EGSR relations by satisfying which the corresponding active quadrants of an object can be obtained. For each object in $\Theta$, we want to estimate the object's active quadrants by evaluating the object and QCN against $M$.
...
\begin{figure}[h!]
\centering\includegraphics[scale=0.35]{BirdImpact.png}\caption{(a) The object in cyan is supported via one-edge support, and the impending impact is indicated by the arrow (b) A subsequent scene after the bird hit (c) The estimated active quadrant (shadowed area)}
\label{BirdImpact}
\end{figure}
...
The vision has the following limitations that can severely affect the matching accuracy: 
\begin{itemize}
...
In Angry Birds, fragmentation is mainly due to the destruction of objects, partially occlusion, and damage. By recognizing these fragments, the system is able to infer whether an object has been destroyed, damaged, or occluded, which leads to robust tracking. We achieve this in three steps:
...
\State $objs \leftarrow$ HandlingFragments($objs$) // add OMBRs created from potential fragments 
\State $solution \leftarrow \{iniobjs, \{\}\}$
\State $domain \leftarrow \{iniobjs, \{\}\}$
\State $domain \leftarrow$ MotionApproximation($iniobjs$, $objs$) 
...
\Return $cobjsList$
\EndProcedure
\Procedure{MotionApproximation}{$iniobjs$, $objs$}
\For {$iniobj \in iniobjs$}
\State $pobjs \leftarrow \{\}$, compute the active quadrants of $iniobj$
\State for each $obj \in objs$, add $obj$ to $pobjs$ if $obj$ is within the quadrants and have the same type with $iniobj$ 
\State $domain \leftarrow domain \cup \{iniobj, \{pobjs\}\}$
\EndFor
\Return $domain$
\EndProcedure
...
\EndProcedure
\Procedure{CommonMotion}{$cobjsList$, $solution$}
