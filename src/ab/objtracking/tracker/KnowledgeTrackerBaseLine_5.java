 ...
 * This class is a back up for fall back solution
 * */
public class KnowledgeTrackerBaseLine_5 extends SMETracker {


	public DirectedGraph<ABTrackingObject, ConstraintEdge> iniGRNetwork, newGRNetwork, iniGRFullNetwork;
	...
	protected long maximum_distance;
	
	public KnowledgeTrackerBaseLine_5(int timegap) {
		super(timegap);
	...
	protected Map<ABTrackingObject, ABTrackingObject> matchObjs(List<ABTrackingObject> moreObjs, List<ABTrackingObject> lessObjs, Map<ABTrackingObject, List<Pair>> morePrefs, Map<ABTrackingObject, List<Pair>> lessPrefs) {
		...
		while (!freeObjs.isEmpty()) {
			...
			else 
			{
				...
				if(pair.sameShape && !moreObj.isDebris && pair.diff < maximum_distance)
				{
	...
	protected boolean isValidMovement(ABTrackingObject obj, ABTrackingObject iniObj, boolean compareMovementType)
	{
		Movement movement = iniObjsMovement.get(iniObj);
		if(movement == null ||
				movement.isValidMovement((int)(obj.getCenterX() - iniObj.getCenterX()), (int)(obj.getCenterY() - iniObj.getCenterY()), false))
 ...
 * */
	protected void isomorphismProcess(DirectedGraph<ABTrackingObject, ConstraintEdge> iniNetwork, DirectedGraph<ABTrackingObject, ConstraintEdge> newNetwork, List<ABTrackingObject> newObjs)
	{
		...
		Set<ABTrackingObject> verticesSet = newNetwork.vertexSet();
		Set<ABTrackingObject> vertices = new HashSet<ABTrackingObject>();
	...
	public static void main(String args[])
	{
		...
		if(filename.contains("_"))
			timegap = Integer.parseInt(filename.substring(filename.indexOf("_")));
		Tracker tracker = new KnowledgeTrackerBaseLine_5(timegap);
