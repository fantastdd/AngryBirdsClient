 ...
 * Assuming downwards gravity
 * */
public class SMDownwardsTracker_2 extends SMETracker_2 {
	...
	public List<ABObject> destroyedObjs = new LinkedList<ABObject>();
 	@Override
	protected float calDiff(ABObject initialObj, ABObject newObj) {
		
		Point center1 = initialObj.getCenter();
		Point center2 = newObj.getCenter();
		...
		{	
			...
			for (ABObject iniObj : initialObjs) {
				if(objType == iniObj.type)
				{
					...
					diffs.add(new Pair(iniObj, calDiff(iniObj, obj), sameShape));
					if (!iniPrefs.containsKey(iniObj)) {
						List<Pair> iniDiffs = new LinkedList<Pair>();
						iniDiffs.add(new Pair(obj, calDiff(iniObj, obj), sameShape));
						iniPrefs.put(iniObj, iniDiffs);
					} else {
						iniPrefs.get(iniObj).add(
								new Pair(obj, calDiff(iniObj, obj), sameShape));
					}
			}
...
/*			for (ABObject occludedObj : currentOccludedObjs)
		...
		/*	for (ABObject _m : matchedObjs.keySet())
			{
				 ABObject iniObj = matchedObjs.get(_m);
