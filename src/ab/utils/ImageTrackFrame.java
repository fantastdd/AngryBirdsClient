...
public class ImageTrackFrame {
	...
	public class ImagePanel extends JPanel implements KeyListener,
		...
		public void paint(Graphics g) {
			...
			if (_img != null) {
				if ((_meta != null) && (_highlightIndex != -1)) {
					BufferedImage canvas = VisionUtils.highlightRegions(_img,
		...
		public void keyPressed(KeyEvent key) {
			...
			if (key.getKeyCode() == KeyEvent.VK_Q) {
				...
				TrackingFrameComparison.goToPrevFrame = true;
				//System.out.println(TrackingFrameComparison.goToNextFrame);
				...
				BufferedImage bi = new BufferedImage(_img.getWidth(null),
						_img.getHeight(null), BufferedImage.TYPE_INT_ARGB);
				...
				if (_meta == null) {
					if (tracker == null)
						...
						for (ABTrackingObject obj : objs) {
							...
							// initialFrame);
							double x = e.getX() * 840 / 1200;
							double y = e.getY() * 480 / 800;
	...
	public ImageTrackFrame(String name, Image img, int[][] meta) {
	...
	public static void main(String args[]) {
		if (args.length > 0)
			ImageTrackFrame.saveFileDir = args[0];
		File file = new File(ImageTrackFrame.saveFileDir);
		if (!file.exists())
			file.mkdir();
		new ActionRobot();
		...
		frame = new ImageTrackFrame(" Screenshots ", screenshot, null);
		while (true) {
			screenshot = ActionRobot.doScreenShot();
