...
public class ImageSegmenter {
    ...
    // Canny Edge Detector
    private boolean[][] findEdgesCanny()
    {     
        // calculate magnitude of gradient in all four directions
        int G[][] = new int[_height][_width];
        int O[][] = new int[_height][_width];
        boolean isEdge[][] = new boolean[_height][_width];
        
        for (int y = _height-3; y > 1; y--)
        for (int x = 2; x < _width - 2; x++)
        {
            double Gx = (_val[y][x+1] - _val[y][x-1]) * 2 +
                        (_val[y-1][x+1] - _val[y-1][x-1]) +
                        (_val[y+1][x+1] - _val[y+1][x-1]);
            double Gy = (_val[y+1][x] - _val[y-1][x]) * 2 +
                        (_val[y+1][x-1] - _val[y-1][x-1]) +
                        (_val[y+1][x+1] - _val[y-1][x+1]);
            
            G[y][x] = (int) Math.sqrt(Gx*Gx + Gy*Gy);
            int theta = (int) Math.toDegrees(Math.atan(Gy / Gx));
            if (theta < 0) theta += 180;
            
            O[y][x] = ((theta+112) / 45) % 4; 
        }
        
        int G2[][] = new int[_height][_width];
        ...
        for (int x = 2; x < _width-2; x++)
        {
            ...
            if (G[y][x] <= G[y1][x1] || G[y][x] < G[y2][x2])
            {
                    G2[y][x] = 0;
            }
        }
